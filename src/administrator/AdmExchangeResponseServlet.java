...
public class AdmExchangeResponseServlet extends HttpServlet {
	...
	// http://localhost:8080/Dormitory/servlet/AdmExchangeResponseServlet?a_id=000001&password=123456&change_code=8000&s_id=201830660178&name=陈晓杰&target_id=201830660174&tname=张三&time=2018:06:24:10:00&agree=0   (1代表同意，0代表不同意)
	@Override
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter out = response.getWriter()) {
			...
			if(verifyResult){    //验证通过才能进行信息查询，返回的是json格式的数据
				if(AdmDAO.admResponseExchangeApply(change_code, agree, time, target_id, tname, s_id, name )) params.put("result", "success");
